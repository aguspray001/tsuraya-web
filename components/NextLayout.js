...
import { linkNavBar } from "../constant";
...
const NextLayout = ({ children, title, description }) => {
  ...
  return (
    <div>
      ...
      {/* <!-- Navbar goes here --> */}
      <header>
        <nav className="fixed z-[99999] bg-white shadow-lg border-2 fixed z-10 w-full">
          <div className="max-w-6xl mx-auto px-4">
            <div className="flex justify-between">
              <div className="flex space-x-7">
                ...
                <div className="hidden items-center space-x-1 md:flex md:gap-x-[2rem]">
                  {linkNavBar.map((linkNav, key) => {
            return(
              <Link href={linkNav.href} key={key}>
                <a className={`py-2 px-2 font-semibold hover:text-blue-500 hover:border-b-4`}>
                {linkNav.title}
                </a>
              </Link>
            )
          ...
          >
            <ul className="">
              <li className="active">
                <a className="block text-sm px-2 py-4 text-white bg-blue-500 font-semibold">
                  <Link href="/">Home</Link>
                </a>
              </li>
              <li>
      ...
      <div className="bg-white overflow-hidden">
        <main className="relative mx-auto px-3 max-w-7xl">{children}</main>
      </div>
      <Footer/>
    </div>
  );
